...
import { imagePlugin } from "../../plugins/types/image"
  ...
  async copy(element: HTMLElement): Promise<void> {
    ...
    try {
      ...
      if (imagePlugin.copyImage) {
        ...
        if (imageUrl) {
          // 내부 클립보드에 저장
          // Type assertion for internal clipboard access
          const plugin = imagePlugin as typeof imagePlugin & { imageClipboard?: string }
          plugin.imageClipboard = imageUrl
  ...
  async paste(element: HTMLElement): Promise<void> {
    ...
    try {
      ...
      if (imagePlugin.pasteImage) {
        ...
        // Type assertion for internal clipboard access
        const plugin = imagePlugin as typeof imagePlugin & { imageClipboard?: string }
        const clipboardImage = plugin.imageClipboard
        if (clipboardImage) {
