...
class PluginStore {
  private models = $state<Map<string, PluginModel>>(new Map())
  
  // 플러그인 모델 생성 또는 가져오기
  getOrCreate(elementId: string, type: string, initialValue?: any): PluginModel {
  ...
  // 플러그인 모델 가져오기
  get(elementId: string): PluginModel | undefined {
  ...
  // 특정 타입의 모든 플러그인 가져오기
  getByType(type: string): PluginModel[] {
    return Array.from(this.models.values()).filter(model => model.type === type)
  }
  
  // 편집 중인 플러그인 가져오기
  getEditingModel(): PluginModel | undefined {
