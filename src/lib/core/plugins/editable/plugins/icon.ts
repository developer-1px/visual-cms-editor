...
const ICON_SETS = {
  lucide: {
    name: 'Lucide',
    icons: [
      'home', 'user', 'settings', 'heart', 'star', 'search', 'plus', 'minus',
      'check', 'x', 'arrow-right', 'arrow-left', 'arrow-up', 'arrow-down',
      'edit', 'trash', 'download', 'upload', 'mail', 'phone', 'map-pin',
      'calendar', 'clock', 'camera', 'image', 'video', 'music', 'book',
      'file', 'folder', 'link', 'share', 'bookmark', 'tag', 'flag'
    ]
  }
};
  ...
  createIconPickerModal(element: HTMLElement): HTMLElement {
    ...
    content.innerHTML = `
      ...
      <div class="grid grid-cols-6 gap-2">
        ${ICON_SETS.lucide.icons.map(icon => `
    ...
    content.querySelectorAll('.icon-option').forEach(button => {
      button.addEventListener('click', () => {
        ...
        element.dispatchEvent(new CustomEvent('iconChanged', {
