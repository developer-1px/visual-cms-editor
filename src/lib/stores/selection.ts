...
export const selectedElements: Readable<string[]> = derived(
  selectionState,
  $state => Array.from($state.selectedElements)
);

export const isAnySelected: Readable<boolean> = derived(
  selectionState,
  $state => $state.selectedElements.size > 0
);

export const editingElement: Readable<string | null> = derived(
  selectionState,
  $state => $state.editingElement
);
