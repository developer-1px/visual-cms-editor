<script lang="ts">
  ...
  $: filteredTemplates =
    selectedCategory === "all" ? defaultTemplates : defaultTemplates.filter((t) => t.category === selectedCategory)
...
{#if isOpen}
  <div class="animate-fade-in fixed inset-0 z-50 flex items-center justify-center bg-black/10 p-4 backdrop-blur-sm">
            ...
            {#each filteredTemplates.filter(t => t && t.id) as template, index (`template-${template.id}-${index}`)}
              ...
              >
                <div class="relative aspect-video overflow-hidden bg-stone-50">
                  <div class="pointer-events-none h-[940px] w-[1250px] origin-top-left scale-[0.08]">
...
{#if showCode && previewTemplate}
  <div class="animate-fade-in fixed inset-0 z-[60] flex items-center justify-center bg-black/20 p-4 backdrop-blur-sm">
    <div class="animate-slide-in max-h-[80vh] w-full max-w-4xl overflow-hidden bg-white shadow-lg">
      ...
      <div class="max-h-[calc(80vh-80px)] overflow-y-auto p-4">
        <pre class="overflow-x-auto bg-stone-50 p-4 text-xs text-stone-800"><code>{convertedCode}</code></pre>
      </div>
    </div>
  </div>
{/if}
