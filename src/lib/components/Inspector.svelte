...
<div>
	{#if hasSelection}
		<div class="animate-fade-in space-y-6">
			...
			<div class="space-y-2">
				{#if selectedType === 'text'}
					<button onclick={editElement} class="btn btn-primary w-full text-sm">
						<Edit2 class="mr-2 h-4 w-4" />
						Edit Text
					</button>
				{/if}
				<button onclick={copyElement} class="btn w-full text-sm">
			...
			{#if elementInfo?.classes}
				<div class="card p-4">
					...
					<code
						class="block border border-stone-200 bg-stone-50 p-3 text-xs break-all text-stone-800"
					>
						{elementInfo.classes}
					</code>
				</div>
